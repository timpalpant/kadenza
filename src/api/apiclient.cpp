...
void ApiClient::request(const QString &tag, const QString &path, const QByteArray &method, const QJsonObject &body)
{
    QUrl url(path);
    if (url.isRelative())
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, tag] {
        ...
        if (error != QNetworkReply::NoError || status < 200 || status >= 300) {
            ...
            const auto errors = document.object().value(QStringLiteral("errors")).toArray();
            if (!errors.isEmpty()) {
