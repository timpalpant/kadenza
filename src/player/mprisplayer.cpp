...
MprisPlayerAdaptor::MprisPlayerAdaptor(PlayerController *player, QObject *parent)
    : QDBusAbstractAdaptor(parent)
    , m_player(player)
{
    ...
    connect(player, &PlayerController::nowPlayingChanged, this, [this] {
        announce({{QStringLiteral("Metadata"), metadata()}, {QStringLiteral("PlaybackStatus"), playbackStatus()}});
    });
    connect(player, &PlayerController::volumeChanged, this, [this] { announce({{QStringLiteral("Volume"), volume()}}); });
    connect(player, &PlayerController::modesChanged, this, [this] {
        announce({{QStringLiteral("Shuffle"), shuffle()}, {QStringLiteral("LoopStatus"), loopStatus()}});
    });
...
void MprisPlayerAdaptor::announce(const QVariantMap &properties)
{
    QDBusMessage changed = QDBusMessage::createSignal(
        QStringLiteral("/org/mpris/MediaPlayer2"), QStringLiteral("org.freedesktop.DBus.Properties"), QStringLiteral("PropertiesChanged"));
    changed << QStringLiteral("org.mpris.MediaPlayer2.Player") << properties << QStringList();
...
QString MprisPlayerAdaptor::playbackStatus() const
{
    ...
    return m_player->title().isEmpty() ? QStringLiteral("Stopped") : QStringLiteral("Paused");
}
...
{
    const QString wanted = status == QStringLiteral("Track")      ? QStringLiteral("one")
...
QDBusObjectPath MprisPlayerAdaptor::trackId() const
{
    ...
    for (const QChar character : id) {
        ...
        const bool usable = (code >= u'a' && code <= u'z') || (code >= u'A' && code <= u'Z') || (code >= u'0' && code <= u'9') || code == u'_';
...
QVariantMap MprisPlayerAdaptor::metadata() const
{
    ...
    result.insert(QStringLiteral("xesam:artist"), QStringList{m_player->artist()});
...
{
    m_player->seek(std::max<qlonglong>(0, m_player->positionMs() + offset / 1000));
    Q_EMIT Seeked(position());
}
void MprisPlayerAdaptor::SetPosition(const QDBusObjectPath &, qlonglong newPosition)
{
