...
QString PlayerController::locateElectron(const QString &sidecar) const
{
    ...
    if (QFileInfo::exists(bundled))
        return bundled;
    // Not every package bundles Electron: the Arch package depends on it
    // instead, to avoid shipping the ~150MB runtime a second time when a
    // system copy already exists. Matched by exact package name rather than a
    // bare "electron" on PATH, because a vanilla Electron build has no
    // Widevine CDM -- playback would silently fall back to 30-second previews
    // instead of failing loudly.
    return QStandardPaths::findExecutable(QString::fromLatin1(kSystemElectron));
}
...
void PlayerController::setDemoState()
{
    ...
    m_lyrics = {QVariantMap{{QStringLiteral("timeMs"), 0}, {QStringLiteral("text"), QStringLiteral("Streetlights dissolve into violet hours")}},
                QVariantMap{{QStringLiteral("timeMs"), 42000}, {QStringLiteral("text"), QStringLiteral("Every window holds a different sky")}},
                QVariantMap{{QStringLiteral("timeMs"), 93000}, {QStringLiteral("text"), QStringLiteral("We follow the afterglow home")}},
                QVariantMap{{QStringLiteral("timeMs"), 151000}, {QStringLiteral("text"), QStringLiteral("Before the colors leave the night")}}};
    m_synchronizedLyrics = true;

    QList<MediaItem> queue;
    for (const auto &[id, title, artist, album, artwork, duration] :
         std::initializer_list<std::tuple<const char *, const char *, const char *, const char *, const char *, qint64>>{
             {"demo-1", "Afterglow", "Mira Vale", "Violet Hours", "violet-hours.svg", 238000},
             {"demo-2", "Glass Gardens", "North Arcade", "Night Transit", "night-transit.svg", 213000},
             {"demo-3", "Blue Current", "The Quiet Shapes", "Tidal Memory", "tidal-memory.svg", 266000}}) {
        MediaItem item;
...
void PlayerController::handleEvent(const QJsonObject &object)
{
    ...
    if (event == QStringLiteral("widevine-ready") || event == QStringLiteral("ready")) {
    ...
    } else if (event == QStringLiteral("session-restore-failed") || event == QStringLiteral("authorization-settled")) {
        if (m_restoringSession) {
        ...
        if (object.value(QStringLiteral("authorized")).toBool() && !m_authenticated) {
            m_authenticated = true;
            Q_EMIT authenticatedChanged();
        }
    } else if (event == QStringLiteral("tokens")) {
        const QString developerToken = object.value(QStringLiteral("developerToken")).toString();
        const QString userToken = object.value(QStringLiteral("musicUserToken")).toString();
        ...
        if (hasTokenPair) {
            Q_EMIT tokensChanged(developerToken, userToken, object.value(QStringLiteral("storefront")).toString(QStringLiteral("us")));
        ...
        m_artwork = MediaItem::artwork(item.value(QStringLiteral("artworkTemplate")).toString());
        m_currentId = item.value(QStringLiteral("catalogId")).toString();
        if (m_currentId.isEmpty())
            m_currentId = item.value(QStringLiteral("id")).toString();
        m_catalogDurationMs = item.value(QStringLiteral("catalogDurationMs")).toInteger();
        m_lyrics.clear();
        ...
        if (!queue.isEmpty())
            handleEvent({{QStringLiteral("event"), QStringLiteral("queue")},
                         {QStringLiteral("items"), queue.value(QStringLiteral("items"))},
                         {QStringLiteral("position"), queue.value(QStringLiteral("position"))}});
        requestLyrics();
        ...
        qInfo().noquote() << "Kadenza playback:"
                          ...
                          << "subscription=" << object.value(QStringLiteral("subscriptionStatus")).toString()
                          << "asset=" << object.value(QStringLiteral("playbackDurationMs")).toInteger()
        ...
        const bool preview = m_durationMs > 0 && m_durationMs <= kLongestPreviewMs && m_catalogDurationMs > m_durationMs + 15000;
        ...
        m_repeat = object.value(QStringLiteral("repeat")).toString(QStringLiteral("none"));
        m_stateDirty = true;
        Q_EMIT modesChanged();
        ...
        for (const auto &value : object.value(QStringLiteral("items")).toArray()) {
            ...
            item.artworkUrl = MediaItem::artwork(source.value(QStringLiteral("artworkTemplate")).toString());
            items.push_back(std::move(item));
            const QString playbackId = source.value(QStringLiteral("catalogId")).toString().isEmpty()
...
bool PlayerController::ready() const
{
    return m_ready;
}
bool PlayerController::authenticated() const
{
    return m_authenticated;
}
bool PlayerController::restoringSession() const
{
    return m_restoringSession;
}
bool PlayerController::playing() const
{
    return m_state == QStringLiteral("playing") || m_state == QStringLiteral("seeking");
}
bool PlayerController::busy() const
{
    return m_state == QStringLiteral("loading") || m_state == QStringLiteral("waiting") || m_state == QStringLiteral("stalled");
}
...
QString PlayerController::artwork() const
{
    return m_artwork;
}
...
QString PlayerController::repeatMode() const
{
    return m_repeat;
}
MediaModel *PlayerController::queueModel()
{
    return &m_queue;
}
int PlayerController::currentQueueIndex() const
{
    return m_currentQueueIndex;
}
QString PlayerController::currentId() const
{
    return m_currentId;
}
QVariantList PlayerController::lyrics() const
{
    return m_lyrics;
}
QString PlayerController::lyricsStatus() const
{
    return m_lyricsStatus;
}
bool PlayerController::synchronizedLyrics() const
{
    return m_synchronizedLyrics;
}
bool PlayerController::previewDetected() const
{
    return m_previewDetected;
}
...
{
    send({{QStringLiteral("cmd"), QStringLiteral("seek")}, {QStringLiteral("positionMs"), positionMs}});
}
void PlayerController::setVolume(double volume)
{
    volume = std::clamp(volume, 0.0, 1.0);
    send({{QStringLiteral("cmd"), QStringLiteral("setVolume")}, {QStringLiteral("volume"), volume}});
}
void PlayerController::setShuffle(bool shuffle)
{
    send({{QStringLiteral("cmd"), QStringLiteral("setShuffle")}, {QStringLiteral("shuffle"), shuffle}});
}
void PlayerController::cycleRepeat()
{
    const QString nextMode = m_repeat == QStringLiteral("none")  ? QStringLiteral("all")
...
{
    const QString safeMode = mode == QStringLiteral("one") || mode == QStringLiteral("all") ? mode : QStringLiteral("none");
    send({{QStringLiteral("cmd"), QStringLiteral("setRepeat")}, {QStringLiteral("mode"), safeMode}});
}
void PlayerController::playSongs(const QStringList &ids, int startIndex)
{
    ...
    if (songs.isEmpty())
        return;
    m_savedQueueIds = ids;
    m_savedQueueIndex = std::clamp(startIndex, 0, static_cast<int>(songs.size()) - 1);
    m_savedPositionMs = 0;
    m_restorePending = false;
    m_stateDirty = true;
    send({{QStringLiteral("cmd"), QStringLiteral("setQueue")},
          {QStringLiteral("songs"), songs},
          {QStringLiteral("startPosition"), std::clamp(startIndex, 0, static_cast<int>(songs.size()) - 1)},
          {QStringLiteral("startPlaying"), true}});
}
void PlayerController::playNext(const QString &id)
{
    if (!id.isEmpty())
        send({{QStringLiteral("cmd"), QStringLiteral("playNext")}, {QStringLiteral("songs"), QJsonArray{id}}});
}
void PlayerController::playLater(const QString &id)
{
    if (!id.isEmpty())
        send({{QStringLiteral("cmd"), QStringLiteral("playLater")}, {QStringLiteral("songs"), QJsonArray{id}}});
}
void PlayerController::playStation(const QString &id)
{
    ...
    m_restorePending = false;
    m_stateDirty = true;
    send({{QStringLiteral("cmd"), QStringLiteral("playStation")}, {QStringLiteral("id"), id}});
}

void PlayerController::playQueueIndex(int index)
{
    send({{QStringLiteral("cmd"), QStringLiteral("changeToIndex")}, {QStringLiteral("index"), index}});
}
void PlayerController::moveQueueItem(int from, int to)
{
    if (from == to || from < 0 || to < 0)
        return;
    send({{QStringLiteral("cmd"), QStringLiteral("moveInQueue")}, {QStringLiteral("from"), from}, {QStringLiteral("to"), to}});
}
void PlayerController::removeQueueItem(int index)
{
    if (index < 0)
        return;
    send({{QStringLiteral("cmd"), QStringLiteral("removeFromQueue")}, {QStringLiteral("index"), index}});
}
...
{
    send({{QStringLiteral("cmd"), QStringLiteral("setVolume")}, {QStringLiteral("volume"), m_volume}});
    send({{QStringLiteral("cmd"), QStringLiteral("setShuffle")}, {QStringLiteral("shuffle"), m_shuffle}});
    send({{QStringLiteral("cmd"), QStringLiteral("setRepeat")}, {QStringLiteral("mode"), m_repeat}});
    ...
    send({{QStringLiteral("cmd"), QStringLiteral("setQueue")},
          {QStringLiteral("songs"), songs},
          {QStringLiteral("startPosition"), std::clamp(m_savedQueueIndex, 0, static_cast<int>(m_savedQueueIds.size()) - 1)},
