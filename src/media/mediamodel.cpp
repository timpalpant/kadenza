...
int MediaModel::playableCount() const {
  return static_cast<int>(
      std::count_if(m_items.cbegin(), m_items.cend(),
                    [](const MediaItem &item) { return item.isPlayable(); }));
}
...
bool MediaModel::loading() const { return m_loading; }
...
QString MediaModel::error() const { return m_error; }
...
QString MediaModel::nextPath() const { return m_nextPath; }
