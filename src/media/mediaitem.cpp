...
bool MediaItem::isSong() const {
  return type == QStringLiteral("songs") ||
...
MediaItem MediaItem::fromJson(const QJsonObject &object) {
  ...
  const bool libraryResource =
      item.type.startsWith(QStringLiteral("library-")) ||
      playParams.value(QStringLiteral("isLibrary")).toBool();
  item.inLibrary = libraryResource;
  item.favorite = attributes.value(QStringLiteral("inFavorites")).toBool() ||
                  attributes.value(QStringLiteral("isFavorited")).toBool();
  ...
  item.explicitContent =
      attributes.value(QStringLiteral("contentRating")).toString() ==
      QStringLiteral("explicit");

  const auto relationships =
      object.value(QStringLiteral("relationships")).toObject();
  const auto relationshipId = [&relationships](const QString &name) {
    const auto data = relationships.value(name)
                          .toObject()
  ...
  const auto catalogData = relationships.value(QStringLiteral("catalog"))
                               .toObject()
                               .value(QStringLiteral("data"))
                               .toArray();
  const QJsonObject catalogObject =
      catalogData.isEmpty() ? QJsonObject() : catalogData.first().toObject();
  ...
  const QString curator =
      attributes.value(QStringLiteral("curatorName")).toString();
  if (!artist.isEmpty()) {
    ...
    const auto genres =
        attributes.value(QStringLiteral("genreNames")).toArray();
    item.subtitle = genres.isEmpty() ? QString() : genres.first().toString();
  }
  QString artworkTemplate = attributes.value(QStringLiteral("artwork"))
                                .toObject()
  ...
  if (!catalogObject.isEmpty()) {
    const auto catalogAttributes =
        catalogObject.value(QStringLiteral("attributes")).toObject();
    if (artworkTemplate.isEmpty()) {
      artworkTemplate = catalogAttributes.value(QStringLiteral("artwork"))
                            .toObject()
                            .value(QStringLiteral("url"))
                            .toString();
    }
