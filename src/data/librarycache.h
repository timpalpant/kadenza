...
public:
  ...
  void upsert(const QString &kind, const QList<MediaItem> &items, qint64 epoch,
              int firstPosition);
  void finishSync(const QString &kind, qint64 epoch);

  [[nodiscard]] QList<MediaItem> items(const QString &kind, Sort sort,
                                       int limit, int offset = 0) const;
  ...
  /// the item requires.
  [[nodiscard]] QString libraryIdFor(const QString &kind,
