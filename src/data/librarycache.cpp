...
const char *const SelectColumns =
    "id, catalog_id, artist_id, album_id, type, title, subtitle, album, "
...
bool LibraryCache::isKnownKind(const QString &kind) {
  return kind == QStringLiteral("albums") || kind == QStringLiteral("songs") ||
         kind == QStringLiteral("artists") ||
         kind == QStringLiteral("playlists") ||
         kind == QStringLiteral("recently-added");
}

bool LibraryCache::available() const { return Database::instance().isOpen(); }
...
qint64 LibraryCache::beginSync(const QString &kind) {
  const qint64 epoch =
      metaValue(QStringLiteral("epoch:") + kind).toLongLong() + 1;
  setMetaValue(QStringLiteral("epoch:") + kind, QString::number(epoch));
  return epoch;
}

void LibraryCache::upsert(const QString &kind, const QList<MediaItem> &items,
                          qint64 epoch, int firstPosition) {
...
void LibraryCache::finishSync(const QString &kind, qint64 epoch) {
  ...
  QSqlQuery query(Database::instance().db());
  query.prepare(QStringLiteral(
      "DELETE FROM library_items WHERE kind = ? AND sync_epoch < ?"));
  query.addBindValue(kind);
  query.addBindValue(epoch);
  query.exec();
  setMetaValue(QStringLiteral("synced_at:") + kind,
               QDateTime::currentDateTimeUtc().toString(Qt::ISODate));
}

QList<MediaItem> LibraryCache::items(const QString &kind, Sort sort, int limit,
                                     int offset) const {
  ...
  const QString order =
      sort == DateAddedDesc
          ? QStringLiteral("ORDER BY (date_added = '') ASC, date_added DESC, "
...
int LibraryCache::count(const QString &kind) const {
  ...
  QSqlQuery query(Database::instance().db());
  query.prepare(
      QStringLiteral("SELECT COUNT(*) FROM library_items WHERE kind = ?"));
  query.addBindValue(kind);
  if (query.exec() && query.next())
...
QString LibraryCache::libraryIdFor(const QString &kind,
                                   const QString &catalogId) const {
  if (!available() || kind.isEmpty() || catalogId.isEmpty())
    return {};
  QSqlQuery query(Database::instance().db());
  query.prepare(QStringLiteral(
      "SELECT id FROM library_items WHERE kind = ? AND in_library = 1 "
...
void LibraryCache::clear() { Database::instance().wipe(); }
