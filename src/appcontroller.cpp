...
namespace {
MediaItem demoItem(const char *id, const char *type, const char *title,
                   const char *subtitle, const char *album, const char *artwork,
                   qint64 duration = 0) {
  MediaItem item;
...
void extractCollection(const QJsonObject &root, const QString &requestedType,
                       QJsonArray &data, QString &next) {
  ...
  const auto relationship =
      resource.value(QStringLiteral("relationships"))
          .toObject()
          .value(requestedType.contains(QStringLiteral("artist"))
...
AppController::AppController(QObject *parent)
  ...
  connect(&m_player, &PlayerController::tokensChanged, this,
          [this](const QString &developer, const QString &user,
            ...
            if (authenticated()) {
              ...
              m_api.get(QStringLiteral("storefront"),
                        QStringLiteral("/v1/me/storefront"));
              refreshHome();
            }
          });
  connect(&m_player, &PlayerController::authenticatedChanged, this,
...
PlayerController *AppController::player() { return &m_player; }
...
MediaModel *AppController::artistsModel() { return &m_artists; }
MediaModel *AppController::playlistsModel() { return &m_playlists; }
MediaModel *AppController::searchModel() { return &m_search; }
MediaModel *AppController::searchSongsModel() { return &m_searchSongs; }
MediaModel *AppController::searchAlbumsModel() { return &m_searchAlbums; }
MediaModel *AppController::searchArtistsModel() { return &m_searchArtists; }
MediaModel *AppController::searchPlaylistsModel() { return &m_searchPlaylists; }
MediaModel *AppController::detailTracksModel() { return &m_detailTracks; }
MediaModel *AppController::stationsModel() { return &m_stations; }
MediaModel *AppController::chartSongsModel() { return &m_chartSongs; }
MediaModel *AppController::chartAlbumsModel() { return &m_chartAlbums; }
MediaModel *AppController::chartPlaylistsModel() { return &m_chartPlaylists; }
MediaModel *AppController::artistTopSongsModel() { return &m_artistTopSongs; }
MediaModel *AppController::artistAlbumsModel() { return &m_artistAlbums; }
MediaModel *AppController::artistSinglesModel() { return &m_artistSingles; }
MediaModel *AppController::artistSimilarModel() { return &m_artistSimilar; }
MediaModel *AppController::artistLatestModel() { return &m_artistLatest; }
QString AppController::detailTitle() const { return m_detailTitle; }
QString AppController::detailSubtitle() const { return m_detailSubtitle; }
QString AppController::detailArtwork() const { return m_detailArtwork; }
QString AppController::detailType() const { return m_detailType; }
int AppController::detailRating() const { return m_detailRating; }
...
bool AppController::detailCollectable() const {
  return (m_detailType.contains(QStringLiteral("albums")) ||
          m_detailType.contains(QStringLiteral("playlists"))) &&
         !(m_detailInLibrary ? m_detailId : m_detailCatalogId).isEmpty();
}

bool AppController::detailInLibrary() const { return m_detailInLibrary; }
...
bool AppController::ratableType(const QString &type) {
  ...
  return resource == QStringLiteral("songs") ||
         resource == QStringLiteral("albums") ||
         resource == QStringLiteral("playlists") ||
...
int AppController::sidebarWidth() const { return m_sidebarWidth; }
...
bool AppController::sidebarCollapsed() const { return m_sidebarCollapsed; }
...
int AppController::artworkSize() const { return m_artworkSize; }
...
bool AppController::loading() const { return m_loadingCount > 0; }
bool AppController::syncing() const { return !m_syncs.isEmpty(); }
...
QStringList AppController::searchHistory() const { return m_searchHistory; }
QStringList AppController::searchHints() const { return m_searchHints; }
bool AppController::searchLibrary() const { return m_searchLibrary; }
...
QString AppController::lastPage() const { return m_lastPage; }
void AppController::setLastPage(const QString &page) {
  if (page.isEmpty() || page == QStringLiteral("login") ||
      page == QStringLiteral("detail") || m_lastPage == page)
    return;
  m_lastPage = page;
  auto config = KSharedConfig::openConfig();
...
void AppController::clearMessage() { setMessage({}); }
void AppController::clearError() {
  setError({});
  m_player.clearError();
}

void AppController::requestModel(const QString &tag, const QString &path,
                                 MediaModel *model, bool append) {
...
void AppController::refreshHome() {
  ...
  requestModel(
      ...
      QStringLiteral("/v1/me/recent/played?types=albums,playlists&limit=10"),
      &m_recent);
  requestModel(
      QStringLiteral("heavy"),
      QStringLiteral(
          "/v1/me/history/heavy-rotation?limit=10&types=albums,playlists"),
      &m_heavyRotation);
  // This endpoint rejects a limit above 10.
  requestModel(QStringLiteral("recent-tracks"),
               QStringLiteral("/v1/me/recent/played/tracks?limit=10"),
               &m_recentTracks);
  m_api.get(QStringLiteral("recommendations"),
            QStringLiteral("/v1/me/recommendations?limit=10"));
}
...
LibraryCache::Sort AppController::sortFor(const QString &uiKind) {
  ...
  return uiKind == kRecentlyAdded ? LibraryCache::DateAddedDesc
                                  : LibraryCache::LibraryOrder;
}
...
void AppController::loadLibrary(const QString &kind, bool refresh) {
  ...
  if (refresh || !m_syncedThisLaunch.contains(cacheKind) ||
      m_cache.count(cacheKind) == 0 ||
      m_cache.isStale(cacheKind, kCacheMaxAgeSeconds))
...
void AppController::loadMore(const QString &kind) {
  ...
  model->appendItems(m_cache.items(cacheKind, sortFor(kind), kCachePageSize,
                                   model->rowCount()));
}
...
void AppController::handleSyncPage(const QString &cacheKind,
  ...
  const bool enough =
      cacheKind == kRecentlyAdded && sync->position >= kRecentlyAddedLimit;
...
void AppController::handleRecommendations(const QJsonDocument &document) {
  ...
  for (const auto &value :
    ...
    const auto attributes =
        recommendation.value(QStringLiteral("attributes")).toObject();
    const QString title = attributes.value(QStringLiteral("title"))
                              .toObject()
                              .value(QStringLiteral("stringForDisplay"))
                              .toString();
...
void AppController::search(const QString &term) {
  ...
  query.addQueryItem(QStringLiteral("types"),
                     QStringLiteral("songs,albums,artists,playlists"));
  requestModel(QStringLiteral("search"),
               QStringLiteral("/v1/catalog/%1/search?%2")
                   .arg(m_api.storefront(), query.toString()),
               &m_search);
}
...
void AppController::requestRatingsFor(MediaModel *model) {
  ...
  for (auto it = byType.cbegin(); it != byType.cend(); ++it) {
    ...
    for (int start = 0; start < ids.size(); start += kRatingsBatchSize) {
      ...
      m_api.get(kRatingsTagPrefix + it.key() + QLatin1Char('-') +
                    QString::number(start),
                QStringLiteral("/v1/me/ratings/%1?ids=%2")
                    .arg(it.key(), batch.join(QLatin1Char(','))));
    }
  }
}
...
void AppController::handleRatings(const QString &, const QJsonDocument &document) {
  for (const auto &value :
    ...
    const int score = rating.value(QStringLiteral("attributes"))
                          .toObject()
...
void AppController::loadStations(bool refresh) {
  ...
  requestModel(QStringLiteral("stations"),
               QStringLiteral("/v1/me/recent/radio-stations?limit=10"),
               &m_stations);
}

void AppController::setRating(const QString &id, const QString &type,
                              int value) {
  ...
  m_api.put(value > 0 ? QStringLiteral("rating-love")
                      : QStringLiteral("rating-dislike"),
            path,
            {{QStringLiteral("type"), QStringLiteral("ratings")},
             {QStringLiteral("attributes"),
              QJsonObject{{QStringLiteral("value"), value > 0 ? 1 : -1}}}});
}
...
void AppController::requestSearchHints(const QString &term) {
  ...
  m_api.get(QStringLiteral("search-hints"),
            QStringLiteral("/v1/catalog/%1/search/hints?%2")
                .arg(m_api.storefront(), query.toString()));
}

void AppController::lookup(const QString &type, const QString &term,
                           const QString &name) {
  ...
  m_api.get(QStringLiteral("lookup"),
            QStringLiteral("/v1/catalog/%1/search?%2")
                .arg(m_api.storefront(), query.toString()));
}
...
void AppController::openAlbumNamed(const QString &album,
                                   const QString &artist) {
  // Album titles repeat across artists, so the artist narrows the match.
  lookup(QStringLiteral("albums"),
         artist.isEmpty() ? album : album + QLatin1Char(' ') + artist, album);
}
...
void AppController::handleLookup(const QJsonDocument &document) {
  const auto data = document.object()
                        ...
                        .toArray();
  if (data.isEmpty()) {
    setError(tr("Could not find “%1” in the Apple Music catalog.")
                 .arg(m_lookupName));
    return;
  }
  const MediaItem item = MediaItem::fromJson(data.first().toObject());
  openDetail(item.id, item.id, m_lookupType, item.title, item.subtitle,
             item.artworkUrl);
  Q_EMIT detailOpened();
}

void AppController::openDetail(const QString &id, const QString &catalogId,
  ...
  if (!m_detailInLibrary) {
    const QString cachedId =
        m_cache.libraryIdFor(cacheKindFor(type), m_detailCatalogId);
    if (!cachedId.isEmpty()) {
  ...
  m_detailRatingId = ratableType(type) && !ratingId.startsWith(QLatin1Char('i'))
                         ? ratingId
                         : QString();
  m_detailRating = 0;
  Q_EMIT detailChanged();
  ...
  if (artist) {
    for (auto *model : {&m_artistTopSongs, &m_artistAlbums, &m_artistSingles,
                        &m_artistSimilar, &m_artistLatest})
      model->clear();
  }
...
QString AppController::collectionPath(const QString &id,
                                      const QString &catalogId,
                                      const QString &type) {
  const QString resourceId = catalogId.isEmpty() ? id : catalogId;
  if (type == QStringLiteral("albums") ||
      (type == QStringLiteral("library-albums") && !catalogId.isEmpty()))
    return QStringLiteral("/v1/catalog/%1/albums/%2?include=tracks")
        .arg(m_api.storefront(), resourceId);
  if (type == QStringLiteral("playlists") ||
      (type == QStringLiteral("library-playlists") && !catalogId.isEmpty()))
    return QStringLiteral("/v1/catalog/%1/playlists/%2?include=tracks")
        .arg(m_api.storefront(), resourceId);
  if (type == QStringLiteral("library-albums"))
    return QStringLiteral("/v1/me/library/albums/%1?include=tracks,catalog")
        .arg(id);
  if (type == QStringLiteral("library-playlists"))
    return QStringLiteral("/v1/me/library/playlists/%1?include=tracks,catalog")
        .arg(id);
  if (type == QStringLiteral("artists"))
    return artistPath(resourceId);
  if (type == QStringLiteral("library-artists"))
    return catalogId.isEmpty()
               ? QStringLiteral(
                     "/v1/me/library/artists/%1?include=albums,catalog")
                     .arg(id)
               : artistPath(catalogId);
  return {};
}
...
QString AppController::artistPath(const QString &catalogArtistId) {
  ...
  return QStringLiteral("/v1/catalog/%1/artists/%2?views=%3")
      .arg(m_api.storefront(), catalogArtistId,
           QStringLiteral("latest-release,top-songs,full-albums,singles,"
                          "similar-artists"));
}

void AppController::playCollection(const QString &id, const QString &catalogId,
                                   const QString &type) {
  const QString path = collectionPath(id, catalogId, type);
  if (path.isEmpty())
    return;
...
void AppController::setInLibrary(const QString &id, const QString &type,
                                 bool inLibrary) {
...
void AppController::addToPlaylist(const QString &playlistId,
  ...
  m_api.post(
      ...
      {{QStringLiteral("data"),
        QJsonArray{
            QJsonObject{{QStringLiteral("id"), songId},
                        {QStringLiteral("type"), QStringLiteral("songs")}}}}});
}

void AppController::loadDemoData() {
  const auto violet = demoItem("album-violet", "albums", "Violet Hours",
                               "Mira Vale", "", "violet-hours.svg");
  const auto transit = demoItem("album-transit", "albums", "Night Transit",
                                "North Arcade", "", "night-transit.svg");
  const auto tidal = demoItem("album-tidal", "albums", "Tidal Memory",
                              "The Quiet Shapes", "", "tidal-memory.svg");
  const auto daylight = demoItem("album-daylight", "albums", "Soft Geometry",
                                 "Daylight Assembly", "", "soft-geometry.svg");
  ...
  m_albums.replaceItems({violet, transit, tidal, daylight,
                         demoItem("album-cloudline", "albums", "Cloudline",
                                  "Juniper Coast", "", "cloudline.svg"),
                         demoItem("album-static", "albums", "Static Bloom",
                                  "Echo Season", "", "static-bloom.svg")});
  m_artists.replaceItems(
      {demoItem("artist-mira", "artists", "Mira Vale", "Alternative", "",
                "violet-hours.svg"),
       demoItem("artist-arcade", "artists", "North Arcade", "Electronic", "",
                "night-transit.svg"),
       demoItem("artist-shapes", "artists", "The Quiet Shapes", "Indie Pop", "",
                "tidal-memory.svg"),
       demoItem("artist-daylight", "artists", "Daylight Assembly", "Ambient",
                "", "soft-geometry.svg")});
  m_playlists.replaceItems(
      {demoItem("playlist-focus", "playlists", "Quiet Focus", "Kanzi", "",
                "soft-geometry.svg"),
       demoItem("playlist-evening", "playlists", "Late Evening", "Kanzi", "",
                "night-transit.svg")});

  QList<MediaItem> songs = {
      demoItem("demo-1", "songs", "Afterglow", "Mira Vale", "Violet Hours",
               "violet-hours.svg", 238000),
      demoItem("demo-2", "songs", "Glass Gardens", "North Arcade",
               "Night Transit", "night-transit.svg", 213000),
      demoItem("demo-3", "songs", "Blue Current", "The Quiet Shapes",
               "Tidal Memory", "tidal-memory.svg", 266000),
      demoItem("demo-4", "songs", "Open Window", "Daylight Assembly",
               "Soft Geometry", "soft-geometry.svg", 194000)};
  m_songs.replaceItems(songs);
  m_search.replaceItems(
      {songs.at(0), violet, songs.at(1), transit,
       demoItem("artist-shapes", "artists", "The Quiet Shapes", "Indie Pop", "",
                "tidal-memory.svg")});
  m_searchSongs.replaceItems({songs.at(0), songs.at(1)});
  m_searchAlbums.replaceItems({violet, transit});
  m_searchArtists.replaceItems(
      {demoItem("artist-shapes", "artists", "The Quiet Shapes", "Indie Pop", "",
                "tidal-memory.svg")});
  m_searchPlaylists.replaceItems(
      {demoItem("playlist-focus", "playlists", "Quiet Focus", "Kanzi", "",
                "soft-geometry.svg")});
}

void AppController::handleSuccess(const QString &tag,
                                  const QJsonDocument &document) {
  ...
  if (tag == QStringLiteral("charts")) {
    ...
    Q_EMIT loadingChanged();
    const auto results =
        document.object().value(QStringLiteral("results")).toObject();
    ...
    const auto firstChart = [&results](const QString &name) {
      ...
      return charts.isEmpty() ? QJsonArray()
                              : charts.first()
                                    .toObject()
                                    .value(QStringLiteral("data"))
                                    .toArray();
  ...
  if (tag == QStringLiteral("search-hints")) {
    QStringList hints;
    const auto terms = document.object()
                           .value(QStringLiteral("results"))
                           .toObject()
                           .value(QStringLiteral("terms"))
                           .toArray();
  ...
  if (tag == QStringLiteral("storefront")) {
    ...
    if (data.isEmpty())
      return;
    const QString id =
        data.first().toObject().value(QStringLiteral("id")).toString();
  ...
  if (tag == QStringLiteral("search")) {
    ...
    const QString prefix =
        m_searchLibrary ? QStringLiteral("library-") : QString();
    const QList<QPair<QString, MediaModel *>> sections = {
        {prefix + QStringLiteral("artists"), &m_searchArtists},
  ...
  } else if (tag == QStringLiteral("detail") &&
             !root.value(QStringLiteral("data")).toArray().isEmpty()) {
    const auto resource =
        root.value(QStringLiteral("data")).toArray().first().toObject();
    const QString type = resource.value(QStringLiteral("type")).toString();
    if (type != QStringLiteral("songs") &&
        type != QStringLiteral("library-songs")) {
      if (m_detailTitle.isEmpty())
        m_detailTitle = resource.value(QStringLiteral("attributes"))
                            .toObject()
  ...
  model->setLoading(false);
  if (tag == QStringLiteral("detail") ||
      tag == QStringLiteral("recent-tracks") ||
      tag == QStringLiteral("stations"))
    requestRatingsFor(model);
  if (m_loadingCount > 0)
    --m_loadingCount;
  Q_EMIT loadingChanged();
...
void AppController::handleFailure(const QString &tag, int status,
  ...
  if (tag == QStringLiteral("search-hints") ||
      tag == QStringLiteral("storefront") ||
      tag == QStringLiteral("recommendations")) {
  ...
  if (tag == QStringLiteral("create-playlist") ||
      tag == QStringLiteral("add-playlist-track")) {
    setError(
        tr("Apple Music request failed (%1): %2").arg(status).arg(message));
    return;
  }
  auto *model = m_pendingModels.take(tag);
