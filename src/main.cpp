...
int main(int argc, char *argv[]) {
  ...
  KAboutData aboutData(QStringLiteral("kanzi"), i18n("Kanzi"),
                       QStringLiteral(KANZI_VERSION_STRING),
                       i18n("A native Apple Music client for KDE Plasma"),
                       KAboutLicense::GPL_V3, i18n("© 2026 the Kanzi authors"));
  aboutData.setOtherText(
      i18n("Kanzi is an unofficial client and is not affiliated with or "
  ...
  aboutData.setBugAddress(
      QByteArrayLiteral("https://github.com/timpalpant/kanzi/issues"));
  ...
  aboutData.addComponent(
      i18n("Electron for Content Security"),
      i18n("castLabs Electron build providing the Widevine CDM"), QString(),
      QStringLiteral("https://github.com/castlabs/electron-releases"));
  KAboutData::setApplicationData(aboutData);
  const QIcon bundledIcon(
      QStringLiteral(":/qt/qml/io/github/timpalpant/kanzi/data/icons/"
                     "io.github.timpalpant.kanzi.svg"));
  QGuiApplication::setWindowIcon(
      QIcon::fromTheme(QStringLiteral(KANZI_APP_ID), bundledIcon));
  ...
  KGlobalAccel::setGlobalShortcut(showShortcut,
                                  QKeySequence(QStringLiteral("Meta+Alt+K")));
  const auto mediaShortcut = [&app](const QString &name, const QString &text,
  ...
  mediaShortcut(QStringLiteral("play-pause"), i18n("Play/Pause"),
                QKeySequence(Qt::Key_MediaTogglePlayPause),
                [controller] { controller->player()->playPause(); });
  mediaShortcut(QStringLiteral("next-track"), i18n("Next Track"),
                QKeySequence(Qt::Key_MediaNext),
                [controller] { controller->player()->next(); });
  mediaShortcut(QStringLiteral("previous-track"), i18n("Previous Track"),
                QKeySequence(Qt::Key_MediaPrevious),
                [controller] { controller->player()->previous(); });

  QObject::connect(controller->player(), &PlayerController::nowPlayingChanged,
                   &app, [controller, mainWindow] {
                     ...
                     KNotification::event(QStringLiteral("trackChanged"),
  ...
  if (!screenshotPath.isEmpty()) {
    ...
    bool widthOk = false;
    bool heightOk = false;
    const int screenshotWidth =
        qEnvironmentVariableIntValue("KANZI_SCREENSHOT_WIDTH", &widthOk);
    const int screenshotHeight =
        qEnvironmentVariableIntValue("KANZI_SCREENSHOT_HEIGHT", &heightOk);
